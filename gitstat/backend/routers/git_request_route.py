...
async def get_github_response(result):
    if result.type == "profile":
        response = await DBHelper.async_http_get(f"https://api.github.com/users/{result.username}")
    elif result.type == "repository":
        response = await DBHelper.async_http_get(f"https://api.github.com/repos/{result.username}/{result.repository}")
    return response
...
async def get_git_info(findby: str):
    try:
        if isint(findby):
            git_info = DBHelper.execute_get_user("get_repository_by_id", findby)
            if git_info:
                git_info['response_type'] = 'Repository'
        else:
            git_info = DBHelper.execute_get_user("get_profile_by_name", findby)
            if git_info:
...
async def renew_git_info(stroke: str, current_user = Depends(get_current_user)):
    try:
        ...
        #     else:
        #         update_repository_history(response)
        #     with DBHelper.get_cursor() as cursor:
        #         cursor.callproc("add_request_to_general_history", [
        #         current_user.id,
        #         response.get('id'),
